  ...
  return (
    <SectionWrapper classBottom={`${main_padding.y}`}>
      <div className="grid md:grid-cols-3 w-max max-w-full lg:gap-10 gap-6 lg:mt-16 md:mt-14 mt-6">
        {data.map((item, idx) => (
          <div key={idx} className="bg-off-white px-6 py-4 rounded-[10px] flex flex-col w-full h-full items-center">
            <Image src={item.icon} alt="" loading="lazy" placeholder="blur" className="h-[100px] w-auto aspect-square object-contain" />
            <h3 className="mt-3 text-primary font-semibold text-2xl">{item.title}</h3>
            <p className="mt-1 text-base text-center leading-[150%]">{item.subtitle}</p>
          </div>
        ))}
      </div>
    </SectionWrapper>
